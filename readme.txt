...
Where to find
=============
To get the source code and images, or to contribute, follow this address:
...
1. Install required software:
    -bcc, nasm and ld86 to build the OS and programs.
...
or as absolute paths starting with '/'. After typing a command, ENTER must be
pressed in order to execute it. There are different types of valid commands:
...
del
Delete a file or directory. One parameter is expected: the path of the file or
directory to delete.
...
It is recommended to download the full source tree and integrate new programs
in the default building scripts. To do so:
-See the building section in this file, and ensure that everything works in the
 development system.
-Place new program source code inside the source/programs directory.
-Modify source/Makefile so new programs are also built, the same way that the
 existing ones.

Template empty program source file:

/*
 * Template empty program.c
 */
...
-The application entry point must be: int entry();
-The string which caused the program to be loaded by the CLI will be pointed by
 a global char* argv, which must be declared somewhere, even if not used.
-"clib/clib.h" should be #included.
-The program must be linked with the clib/clib.o, clib/x86.o and clib/entry.o
 generated with the same version of the OS that will run the program, and
 specify 0xD000 as start text address.
-The final binary file should be flat 16-bit without header or symbols.

Final notes
===========
Please, contact vialamo@gmail.com for more information or questions.
